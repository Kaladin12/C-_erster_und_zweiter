...
void fightScreen(int opponent, int type, int potions) {
	system("cls");
	locale::global(std::locale("C"));
	string states[2] = { "Puntaje: ", "Points: " };
	int item = 0;
	...
	gotoxy(65, 1); cout << gameVar.username;
	clearTitleBox();
	gotoxy(6, 2); cout << states[language] << gameVar.score << enemySequence[opponent].status; gotoxy(35, 2); cout << enemySequence[opponent].name[language]; gotoxy(65, 2); cout << "HP: " << enemySequence[opponent].hp;
	for (int i = 0; i < 19; i++) {
		gotoxy(20, 5 + i);
...
void Transition_Screen(int opponent, int tipe, int character) {
	...
	string enemyMessage[2][7] = { {"LO SIENTO DON CANGREJOOOOOOOOOO!!!!","Yo solo quería lasaña. :(","Joooojooo...jooojo....jo.....","Dónde está el amor?","NOO, MARGEEEEEEEEEEE","Don Cangrejo no me paga lo suficiente para esto.","(MICKEY) Volveré. Pronto. Y LOS PUGARÉ A TODOS."},
	...
	if (tipe == 0 || tipe == 1) {
		if (tipe == 0) {//Mataste A
			gotoxy(30, 2); cout << message[tipe][language] << " " << character + 1;
			for (int i = 0; i < 19; i++) {
				gotoxy(20, 5 + i);
				cout << personajes::imagen[currentPlayer[character].id][i];
			}
