...
int mayor = -1,mayor_1=-1, jug = -1; bool out = false, seis=false;
...
void Escaleras(int jugador, int numcarta) {
	...
	for (int indice = 0; indice < (13 - numcarta); indice++) {
		...
		if (indice_inicial == 8) {
			cout << "Escalera Chida" << endl; gofish.Ganadores[jugador][6] = true;
		}
		else {
			cout << "Escalera" << endl; gofish.Ganadores[jugador][5] = true;
		}
	}
}
...
void SalirSeisCartas(int jugador) {
	...
	/*if (gofish.Jugadores[jugador][0] == gofish.Jugadores[jugador][1]|| gofish.Jugadores[jugador][0] == gofish.Jugadores[jugador][2]|| gofish.Jugadores[jugador][0] == gofish.Jugadores[jugador][3]|| gofish.Jugadores[jugador][0] == gofish.Jugadores[jugador][4]|| gofish.Jugadores[jugador][0] == gofish.Jugadores[jugador][5])
		contador += 1;
	if (gofish.Jugadores[jugador][1] == gofish.Jugadores[jugador][2]|| gofish.Jugadores[jugador][1] == gofish.Jugadores[jugador][3]|| gofish.Jugadores[jugador][1] == gofish.Jugadores[jugador][4]|| gofish.Jugadores[jugador][1] == gofish.Jugadores[jugador][5])
		contador += 1;
	if (gofish.Jugadores[jugador][2] == gofish.Jugadores[jugador][3] || gofish.Jugadores[jugador][2] == gofish.Jugadores[jugador][4] || gofish.Jugadores[jugador][2] == gofish.Jugadores[jugador][5])
		contador += 1;
	if (gofish.Jugadores[jugador][3] == gofish.Jugadores[jugador][4]|| gofish.Jugadores[jugador][3] == gofish.Jugadores[jugador][5])
		contador += 1;
	if (gofish.Jugadores[jugador][4] == gofish.Jugadores[jugador][5])
		contador += 1;
	if (contador == 3) {
		out = true; jug = jugador; seis = true;
	}*/
		...
		while (out==false) {
			...
			if (baraja_o_pool == "C") {
				...
				while (sindice != indice) {
					...
					Dejar_Carta(sindice); sindice += 1; if (sindice >= respuesta) sindice = 0; system("cls");
				} out = true;
				break; 
			}
			indice += 1; system("cls");
